...
public:
    ...
    WXDLLIMPEXP_BASE wxUniChar() : m_value(0) {}

    ~wxUniChar() {}

    // Create the character from 8bit character value encoded in the current
    // locale's charset.
    WXDLLIMPEXP_BASE wxUniChar(char c) { m_value = From8bit(c); }
    WXDLLIMPEXP_BASE wxUniChar(unsigned char c) { m_value = From8bit((char)c); }

    // Create the character from a wchar_t character value.
#if wxWCHAR_T_IS_REAL_TYPE
    WXDLLIMPEXP_BASE wxUniChar(wchar_t c) { m_value = c; }
#endif

    WXDLLIMPEXP_BASE wxUniChar(int c) { m_value = c; }
    WXDLLIMPEXP_BASE wxUniChar(unsigned int c) { m_value = c; }
    WXDLLIMPEXP_BASE wxUniChar(long int c) { m_value = c; }
    WXDLLIMPEXP_BASE wxUniChar(unsigned long int c) { m_value = c; }
    WXDLLIMPEXP_BASE wxUniChar(short int c) { m_value = c; }
    WXDLLIMPEXP_BASE wxUniChar(unsigned short int c) { m_value = c; }

    WXDLLIMPEXP_BASE wxUniChar(const wxUniCharRef& c);

    // Returns Unicode code point value of the character
    WXDLLIMPEXP_BASE value_type GetValue() const { return m_value; }

#if wxUSE_UNICODE_UTF8
    // buffer for single UTF-8 character
    struct WXDLLIMPEXP_BASE Utf8CharBuffer
    {
        char data[5];
    ...
    WXDLLIMPEXP_BASE Utf8CharBuffer AsUTF8() const;
#endif // wxUSE_UNICODE_UTF8

    // Returns true if the character is an ASCII character:
    WXDLLIMPEXP_BASE bool IsAscii() const { return m_value < 0x80; }
    ...
    // must be non-NULL)
    WXDLLIMPEXP_BASE bool GetAsChar(char *c) const
    {
#if wxUSE_UNICODE
        if ( !IsAscii() )
        {
    ...
    // functions
    WXDLLIMPEXP_BASE operator char() const { return To8bit(m_value); }
    WXDLLIMPEXP_BASE operator unsigned char() const { return (unsigned char)To8bit(m_value); }
#if wxWCHAR_T_IS_REAL_TYPE
    WXDLLIMPEXP_BASE operator wchar_t() const { return (wchar_t)m_value; }
#endif
    WXDLLIMPEXP_BASE operator int() const { return (int)m_value; }
    WXDLLIMPEXP_BASE operator unsigned int() const { return (unsigned int)m_value; }
    WXDLLIMPEXP_BASE operator long int() const { return (long int)m_value; }
    WXDLLIMPEXP_BASE operator unsigned long int() const { return (unsigned long)m_value; }
    WXDLLIMPEXP_BASE operator short int() const { return (short int)m_value; }
    WXDLLIMPEXP_BASE operator unsigned short int() const { return (unsigned short int)m_value; }
    ...
    WXDLLIMPEXP_BASE operator bool() const { return m_value != 0; }
    WXDLLIMPEXP_BASE bool operator!() const { return !((bool)*this); }

    // And this one is needed by some (not all, but not using ifdefs makes the
    // code easier) compilers to parse "str[0] && *p" successfully
    WXDLLIMPEXP_BASE bool operator&&(bool v) const { return (bool)*this && v; }

    // Assignment operators:
    WXDLLIMPEXP_BASE wxUniChar& operator=(const wxUniChar& c) { if (&c != this) m_value = c.m_value; return *this; }
    WXDLLIMPEXP_BASE wxUniChar& operator=(const wxUniCharRef& c);
    WXDLLIMPEXP_BASE wxUniChar& operator=(char c) { m_value = From8bit(c); return *this; }
    WXDLLIMPEXP_BASE wxUniChar& operator=(unsigned char c) { m_value = From8bit((char)c); return *this; }
#if wxWCHAR_T_IS_REAL_TYPE
    WXDLLIMPEXP_BASE wxUniChar& operator=(wchar_t c) { m_value = c; return *this; }
#endif
    WXDLLIMPEXP_BASE wxUniChar& operator=(int c) { m_value = c; return *this; }
    WXDLLIMPEXP_BASE wxUniChar& operator=(unsigned int c) { m_value = c; return *this; }
    WXDLLIMPEXP_BASE wxUniChar& operator=(long int c) { m_value = c; return *this; }
    WXDLLIMPEXP_BASE wxUniChar& operator=(unsigned long int c) { m_value = c; return *this; }
    WXDLLIMPEXP_BASE wxUniChar& operator=(short int c) { m_value = c; return *this; }
    WXDLLIMPEXP_BASE wxUniChar& operator=(unsigned short int c) { m_value = c; return *this; }

    // Comparison operators:

    // define the given comparison operator for all the types
#define wxDEFINE_UNICHAR_OPERATOR(op)                                         \
    WXDLLIMPEXP_BASE bool operator op(const wxUniChar& c) const { return m_value op c.m_value; }\
    WXDLLIMPEXP_BASE bool operator op(char c) const { return m_value op From8bit(c); }         \
    WXDLLIMPEXP_BASE bool operator op(unsigned char c) const { return m_value op From8bit((char)c); } \
    wxIF_WCHAR_T_TYPE( WXDLLIMPEXP_BASE bool operator op(wchar_t c) const { return m_value op (value_type)c; } )    \
    WXDLLIMPEXP_BASE bool operator op(int c) const { return m_value op (value_type)c; }        \
    WXDLLIMPEXP_BASE bool operator op(unsigned int c) const { return m_value op (value_type)c; }        \
    WXDLLIMPEXP_BASE bool operator op(short int c) const { return m_value op (value_type)c; }  \
    WXDLLIMPEXP_BASE bool operator op(unsigned short int c) const { return m_value op (value_type)c; }  \
    WXDLLIMPEXP_BASE bool operator op(long int c) const { return m_value op (value_type)c; }   \
    WXDLLIMPEXP_BASE bool operator op(unsigned long int c) const { return m_value op (value_type)c; }
...
#undef wxDEFINE_UNICHAR_OPERATOR
    ...
    WXDLLIMPEXP_BASE int operator-(const wxUniChar& c) const { return m_value - c.m_value; }
    WXDLLIMPEXP_BASE int operator-(char c) const { return m_value - From8bit(c); }
    WXDLLIMPEXP_BASE int operator-(unsigned char c) const { return m_value - From8bit((char)c); }
    WXDLLIMPEXP_BASE int operator-(wchar_t c) const { return m_value - (value_type)c; }
...
private:
    ...
    WXDLLIMPEXP_BASE static value_type FromHi8bit(char c);
    WXDLLIMPEXP_BASE static char ToHi8bit(value_type v);
    WXDLLIMPEXP_BASE static bool GetAsHi8bit(value_type v, char *c);
